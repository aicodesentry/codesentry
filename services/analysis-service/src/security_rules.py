...
SECURITY_RULES: List[SecurityRule] = [
    ...
    SecurityRule(
        ...
        pattern=re.compile(r"(open\(|send_file\(|File\(|readFile\().*(\.\./|req\.|input\()", re.IGNORECASE),
    ...
    SecurityRule(
        ...
        confidence=0.88,
        exploitability="high",
        pattern=re.compile(r"(pickle\.loads|yaml\.load\(|eval\(|marshal\.loads)", re.IGNORECASE),
        description="Deserialization primitive can execute attacker-controlled payloads.",
        remediation="Use safe loaders and strict schema validation on untrusted input.",
    ),
    SecurityRule(
